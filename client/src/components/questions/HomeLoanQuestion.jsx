...
const INTEREST_PRESETS = [
  ...
  { label: '₹4L', value: 4_00_000 },
  { label: '₹6L', value: 6_00_000 },
  { label: '₹8L', value: 8_00_000 },
];

export default function HomeLoanQuestion({ answers, onNext }) {
  const existing = answers.homeLoanInterest;
  ...
  const [selectedPreset, setSelectedPreset] = useState(
    hasExisting
      ? INTEREST_PRESETS.find((p) => p.value === existing)?.value ?? null
      : null
  );
  ...
  function handleContinue() {
    if (amount > 0) onNext({ homeLoanInterest: amount });
  }
    ...
    <div className="flex-1 flex flex-col">
      <div className="flex-1 px-5 pt-4">
        ...
        <p className="text-xs text-(--color-muted) mt-1">
          up to ₹2L deductible under section 24b
        </p>
        ...
        {!showAmount ? (
          ...
          <div className="mt-6 animate-slide-in">
            <div className="text-[10px] tracking-[0.15em] text-(--color-muted) uppercase mb-3">
            ...
            {amount > 0 && (
              <p className="text-xs text-(--color-muted) mt-1.5 ml-5">
                {formatINR(amount)} per year
              </p>
            )}
            <p className="text-[10px] text-(--color-muted) mt-2">
              check your loan app -- typically ₹2-8L/year
            </p>
          </div>
        )}
      </div>

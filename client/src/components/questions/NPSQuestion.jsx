...
export default function NPSQuestion({ answers, onNext }) {
  const existing = answers.npsContribution;
  ...
  const [selectedPreset, setSelectedPreset] = useState(
    hasExisting
      ? NPS_PRESETS.find((p) => p.value === existing)?.value ?? null
      : null
  );
  ...
  function handleContinue() {
    if (amount > 0) onNext({ npsContribution: amount });
  }
          ...
          <div className="mt-6 animate-slide-in">
            ...
            {amount > 0 && (
              <p className="text-xs text-(--color-muted) mt-1.5 ml-5">
                ...
                {amount > 50_000 && (
                  <span className="text-(--color-amber)"> (max deduction ₹50K)</span>
                )}
              </p>
            )}
          </div>
